...
local function check_ardrive_cli()
    local success, _ = run_command("ardrive --version")
    return success
end
...
if not build_success then
    print("Error: bun run build failed. Output:")
    print(build_result)
    os.exit(1)
else
    print("bun run build completed successfully")
end

-- Check if ArDrive CLI is available
if not check_ardrive_cli() then
    print("Error: ArDrive CLI is not installed or not in the system PATH.")
    print("Please install ArDrive CLI and make sure it's accessible from the command line.")
    print("You can install it using: npm install -g ardrive-cli")
    print("Build artifacts are in the 'dist' directory. Upload them manually to ArDrive.")
    os.exit(1)
end
...
local upload_success, upload_result = run_command(upload_command)
print("Upload result:")
print(upload_result)

-- Check if upload_result is nil or empty
if not upload_success or not upload_result or upload_result == "" then
    print("Error: ArDrive command failed or returned no output")
    os.exit(1)
end
...
if success then
    ...
    local manifest_upload_command = string.format([[
        ardrive upload-file --replace --content-type "application/x.arweave-manifest+json" --local-path "%s" --parent-folder-id "%s" -w %s --metadata-gql-tags "App-Name" "tinyNav-builds" --metadata-gql-tags "App-Version" "%s" --metadata-gql-tags "Build-Type" "%s" --metadata-gql-tags "Build-ID" "%s"
    ]], manifest_file, config.parentFolderId, config.walletPath, new_version,
        is_prod_build and "production" or "development", build_id)

    local manifest_upload_success, manifest_upload_result_raw = run_command(manifest_upload_command)
    ...
    if manifest_upload_success then
        ...
        if manifest_upload_result.created and manifest_upload_result.created[1] then
            ...
            print("Website Link:")
            print("https://permagate.io/" .. manifest_upload_result.created[1].dataTxId)
        else
        ...
        local upload_state_upload_command = string.format([[
            ardrive upload-file --replace --local-path "%s" --parent-folder-id "%s" -w %s --metadata-gql-tags "App-Name" "tinyNav-states" --metadata-gql-tags "App-Version" "%s" --metadata-gql-tags "Build-Type" "%s" --metadata-gql-tags "Build-ID" "%s"
        ]], upload_state_file, config.parentFolderId, config.walletPath, new_version,
            is_prod_build and "production" or "development", build_id)

        local upload_state_upload_success, upload_state_upload_result_raw = run_command(upload_state_upload_command)
